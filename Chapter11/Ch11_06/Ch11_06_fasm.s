...
;------------------------------------------------------------------------------
; void MatVecMulF32_avx2(Vec4x1_F32* vec_b, const float* m,
;   const Vec4x1_F32* vec_a, size_t num_vec);
;------------------------------------------------------------------------------

        section .text

        global MatVecMulF32_avx2
MatVecMulF32_avx2:
...
; Initialize
        ...
        vmovaps ymm0,[rsi]                  ;xmm0 = m row 0
        vextractf128 xmm1,ymm0,1            ;xmm1 = m ro1 1
        vmovaps ymm2,[rsi+32]               ;xmm2 = m row 2
...
Loop1:  add rax,16
        ...
        vbroadcastss xmm5,[rdx+rax]         ;xmm5 = vec_a[i].W
        vmulps xmm4,xmm0,xmm5               ;xmm4  = m_T row 0 * W vals

        vbroadcastss xmm5,[rdx+rax+4]       ;xmm5 = vec_a[i].X
        vfmadd231ps xmm4,xmm1,xmm5          ;xmm4 += m_T row 1 * X vals

        vbroadcastss xmm5,[rdx+rax+8]       ;xmm5 = vec_a[i].Y
        vfmadd231ps xmm4,xmm2,xmm5          ;xmm4 += m_T row 2 * Y vals

        vbroadcastss xmm5,[rdx+rax+12]      ;xmm5 = vec_a[i].Z
        vfmadd231ps xmm4,xmm3,xmm5          ;xmm4 += m_T row 3 * Z vals

        vmovaps [rdi+rax],xmm4              ;save vec_b[i]

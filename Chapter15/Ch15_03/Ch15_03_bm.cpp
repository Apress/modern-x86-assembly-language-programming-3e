...
void Convolve1D_Ks5_F32_bm(void)
{
    std::cout << "\nRunning benchmark function Convolve1D_Ks5_F32_bm() - please wait\n";
    ...
    {
        bmtt.Start(i, 0);
        Convolve1D_Ks5_F32_cpp(y0.data(), x.data(), kernel.data(), x.size());
        bmtt.Stop(i, 0);

        bmtt.Start(i, 1);
        Convolve1D_Ks5_F32_avx512(y1.data(), x.data(), kernel.data(), x.size());
        bmtt.Stop(i, 1);
    }

    std::string fn = bmtt.BuildCsvFilenameString("@Ch15_03_Convolve1D_Ks5_F32_bm");

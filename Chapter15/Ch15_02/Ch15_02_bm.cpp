...
void Convolve1D_F64_bm(void)
{
    ...
    {
        bmtt.Start(i, 0);
        Convolve1D_F64_cpp(y0.data(), x.data(), kernel.data(), x.size(), kernel.size());
        bmtt.Stop(i, 0);

        bmtt.Start(i, 1);
        Convolve1D_F64_avx512(y1.data(), x.data(), kernel.data(), x.size(), kernel.size());
        bmtt.Stop(i, 1);
    }

    std::string fn = bmtt.BuildCsvFilenameString("@Ch15_02_Convolve1D_F64_bm");

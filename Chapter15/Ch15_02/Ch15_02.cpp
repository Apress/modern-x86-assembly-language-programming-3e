...
{
    constexpr char nl = '\n';
    const char* bn_results = "@Ch15_02_Convolve1D_F64_Output";

    std::cout << "----- Results for Ch15_02 -----\n";
    std::cout << "Executing Convolve1D_F64()" << nl;

    constexpr int64_t num_pts = 67;
    const std::vector<double> kernel{ 0.0625f, 0.25f, 0.375f, 0.25f, 0.0625f };
    ...
    bool rc1 = Convolve1D_F64_cpp(y1.data(), x.data(), kernel.data(), x.size(), kernel.size());
    bool rc2 = Convolve1D_F64_avx512(y2.data(), x.data(), kernel.data(), x.size(), kernel.size());
...
int main(void)
{
    ...
    {
        std::cout << "Ch15_02 exception: " << ex.what() << '\n';
    }

...
;------------------------------------------------------------------------------
; void MatrixMulF64_avx512(float* c, const float* a, const float* b,
;   const size_t* sizes);
;------------------------------------------------------------------------------
...
SF          equ 8                           ;scale factor for F64 elements
        ...
        .code
MatrixMulF64_avx512 proc frame
...
Loop2:  lea r11,[r9+NSE]                        ;r11 = j + NSE
        ...
        vxorpd zmm2,zmm2,zmm2                   ;initialize packed c_vals
        mov r10,r15                             ;r10 = a_ncols

; Calculate c[i][j:j+NSE-1]
        align 16
Loop3a: vbroadcastsd zmm0,real8 ptr [rbx]       ;broadcast a[i][k]
        vfmadd231pd zmm2,zmm0,[rsi]             ;zmm2 += a[i][k] * b[k][j:j+NSE-1]
...
; Save c[i][j:j+NSE-1]
        vmovupd zmmword ptr [rcx],zmm2          ;save c[i][j:j+NSE-1]
...
; Calculate c[i][j:j+NRC] (NRC is num_residual_cols)
        align 16
Loop3b: vbroadcastsd zmm0,real8 ptr [rbx]       ;broadcast a[i][k]
        vmovupd zmm1{k1},zmmword ptr [rsi]      ;load b[k][j:j+NRC]
        vfmadd231pd zmm2,zmm0,zmm1              ;zmm2 += a[i][k] * b[k][j:j+NRC]
...
; Save c[i][j:j+NRC]
        vmovupd zmmword ptr [rcx]{k1},zmm2      ;save c[i][j:j+NRC]

...
; Calculate c[i][j:j+NSE-1]
        align 16
Loop3a: vbroadcastsd zmm0,[rbx]                 ;broadcast a[i][k]
...
; Save c[i][j:j+NSE-1]
        vmovupd [rcx],zmm2                      ;save c[i][j:j+NSE-1]
...
; Calculate c[i][j:j+NRC] (NRC is num_residual_cols)
        align 16
Loop3b: vbroadcastsd zmm0,[rbx]                 ;broadcast a[i][k]
        vmovupd zmm1{k1},[rsi]                  ;load b[k][j:j+NRC]
...
; Save c[i][j:j+NRC]
        vmovupd [rcx]{k1},zmm2                  ;save c[i][j:j+NRC]

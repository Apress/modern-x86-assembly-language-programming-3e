...
// This namespace contain miscellaneous inline functions.
namespace MF
{
    inline std::string BuildHostFilenameString(const char* s)
    {
        std::string fn = s;
        auto n = fn.rfind('.');
        ...
        {
            fn.append(us);
            fn.append(OS::GetComputerName());
        }
        else
        {
            fn.insert(n, us);
            fn.insert(n + 1, OS::GetComputerName());
        }
    ...
    inline std::string GetFnDT(void)
    {
        std::chrono::system_clock::time_point now = std::chrono::system_clock::now();

        char b1[100];
        const char* fmt1 = "%F_%H-%M-%S";
        std::time_t tt = std::chrono::system_clock::to_time_t(now);

...
template <typename T> void GenSignal1D(std::vector<T>& x, unsigned int rng_seed)
{
    ...
    constexpr double t_step = 0.002;
    constexpr size_t m = 3;
    constexpr double amp[m] {1.0, 0.80, 1.20};
    constexpr double freq[m] {5.0, 10.0, 15.0};
    constexpr double phase[m] {0.0, 45.0, 90.0};
...
template <typename T>
std::string SaveResults1D(const char* bn,
    const std::vector<std::vector<T>*>& signal_vectors,

...
public:
    ...
    static void Add4x4(MatrixF64& result, const MatrixF64& mat1, const MatrixF64& mat2)
    {
#if defined(_DEBUG)
        if (mat1.m_NumRows != 4 || mat1.m_NumCols != 4 || mat2.m_NumRows != 4 || mat2.m_NumCols != 4)
            throw std::runtime_error("Size check #1 failed - MatrixF64::Addl4x4");

        if (result.m_NumRows != 4 || result.m_NumCols != 4)
            throw std::runtime_error("Size check #2 failed - MatrixF64::Add4x4");
#endif
    ...
    static void Mul4x4(MatrixF64& result, const MatrixF64& mat1, const MatrixF64& mat2)
    {
#if defined(_DEBUG)
        if (mat1.m_NumRows != 4 || mat1.m_NumCols != 4 || mat2.m_NumRows != 4 || mat2.m_NumCols != 4)
            throw std::runtime_error("Size check #1 failed - MatrixF64::Mul4x4");

        if (result.m_NumRows != 4 || result.m_NumCols != 4)
            throw std::runtime_error("Size check #2 failed - MatrixF64::Mul4x4");
#endif
        ...
        for (size_t i = 0; i < nrows; i++)
        {
            for (size_t j = 0; j < ncols; j++)
            {
                ...
                for (size_t k = 0; k < m; k++)
                {
                    double val = mat1.m_Data[i * ncols + k] * mat2.m_Data[k * ncols + j];
                    sum += val;
                }

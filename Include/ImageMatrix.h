...
public:
    ...
    ImageMatrix(std::string& filename, PixelType pixel_type, Channel channel = Channel::None)
    {
        if (pixel_type != PixelType::Gray8 && pixel_type != PixelType::Rgb32)
            throw std::runtime_error("ImageMatrix::LoadImage() - unsupported pixel_type");

#if defined(_MSC_VER)
        ImageMatrixW::LoadImage(m_ImageBuffer, filename.c_str(), pixel_type, channel);
#endif

#if defined(__GNUG__)
        ImageMatrixL::LoadImage(m_ImageBuffer, filename.c_str(), pixel_type, channel);
#endif
    ...
    void SaveImage(std::string& filename, ImageFileType file_type)
    {
#if defined(_MSC_VER)
        ImageMatrixW::SaveImage(m_ImageBuffer, filename.c_str(), file_type);
#endif

#if defined(__GNUG__)
        ImageMatrixL::SaveImage(m_ImageBuffer, filename.c_str(), file_type);
#endif
    }

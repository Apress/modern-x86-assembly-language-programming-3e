...
HistoOffsets    dd  256 *  0, 256 *  1, 256 *  2, 256 *  3
                ...
                dd  256 *  8, 256 *  9, 256 * 10, 256 * 11
                dd  256 * 12, 256 * 13, 256 * 14, 256 * 15
...
HS      equ     256 * 4                         ;size of histogram in bytes
        ...
        global BuildHistogram_avx512
BuildHistogram_avx512:
        mov r11,rsp                             ;save copy of rsp

; Validate arguments
        mov r10,rdi                             ;save copy of histo
...
; Initialize
        ...
        vmovdqa32 zmm17,[HistoOffsets]          ;zmm17 = local histogram offsets
...
; Gather entries from local histograms (i.e., histo0[pb[i]] - histo15[pb[i+15]])
        vpmovzxbd zmm1,[rsi+rax]                ;load pb[i:i+15] as dwords
...
Loop2:  add rax,16                              ;rax += 16
        vmovdqa32 zmm0,[rsp+rax*4 + HS*0]       ;load histo0[i:i+15]
        vmovdqa32 zmm1,[rsp+rax*4 + HS*1]       ;load histo1[i:i+15]
        vpaddd zmm0,zmm0,[rsp+rax*4 + HS*2]     ;add histo2[i:i+15]
        vpaddd zmm1,zmm1,[rsp+rax*4 + HS*3]     ;add histo3[i:i+15] 
        vpaddd zmm0,zmm0,[rsp+rax*4 + HS*4]     ;add histo4[i:i+15] 
        vpaddd zmm1,zmm1,[rsp+rax*4 + HS*5]     ;add histo5[i:i+15] 
        vpaddd zmm0,zmm0,[rsp+rax*4 + HS*6]     ;add histo6[i:i+15] 
        vpaddd zmm1,zmm1,[rsp+rax*4 + HS*7]     ;add histo7[i:i+15] 
        vpaddd zmm0,zmm0,[rsp+rax*4 + HS*8]     ;add histo8[i:i+15] 
        vpaddd zmm1,zmm1,[rsp+rax*4 + HS*9]     ;add histo9[i:i+15]
        vpaddd zmm0,zmm0,[rsp+rax*4 + HS*10]    ;add histo10[i:i+15]
        vpaddd zmm1,zmm1,[rsp+rax*4 + HS*11]    ;add histo11[i:i+15] 
        vpaddd zmm0,zmm0,[rsp+rax*4 + HS*12]    ;add histo12[i:i+15] 
        vpaddd zmm1,zmm1,[rsp+rax*4 + HS*13]    ;add histo13[i:i+15] 
        vpaddd zmm0,zmm0,[rsp+rax*4 + HS*14]    ;add histo14[i:i+15]
        vpaddd zmm1,zmm1,[rsp+rax*4 + HS*15]    ;add histo15[i:i+15]
        vpaddd zmm2,zmm0,zmm1                   ;zmm2 = histo[i:i+15]

        vmovdqa32 [r10+rax*4],zmm2              ;save histo[i:i+15]
